...
$prod_id = !empty($_POST['prod_id'])?$_POST['prod_id']:NULL; 
...
$success = 'Регистрация повторной скупки прошла успешно';
if ($worker && $serial_number && $prod_id && $product && $weight && $pay && $cash_card && $_SESSION['reload'] != 'on') {
	//Регестрируем запись
    ...
    //Добавляем дополнительные работы к изделию
    $page_product = $pages->get('id=' . $prod_id . '');
    foreach ($page_product->addw_table as $addw) {
        $addw_add = $pages->get('template=product_itm, id=' . $operation_id . '')->addw_table->getNew();
        $addw_add->description_operation = $addw->description_operation;
        $addw_add->sum = $addw->sum;
        $addw_add->save();
    }

    //Записываем регистрацию  в лог
    $log = '';
    $log = 'ОФОРМЛЕНИЕ ПОВТОРНОЙ СКУПКИ ИЗДЕЛИЯ: ';
    ...
    file_put_contents(__DIR__ . '/log_products.txt', $log . PHP_EOL, FILE_APPEND);
                ...
                $log .= 'Операция проведена: ' . $worker . ', ID записи: ' . $cash_operation_id . ', Сумма: ' . $pay . ', Вид платежа: ' . $cash_card . ', Описание: Расход при скупке изделия по операции ID: ' . $operation_id;
	...
	$success = 'Регистрация повторной скупки не прошла!<br>Ошибка в данных';
...
?>
    <div id="content" style="max-width: 700px;">
    	<h1 class="uk-heading-hero uk-text-center">Скупка изделия - Регистрация повторной скупки</h1>
...
<div id="content">
	<h1 class="uk-margin-remove uk-heading-hero uk-text-center">Скупка изделия - Регистрация повторной скупки</h1>
	<div>
        ...
        <div>
            <h3 class="uk-card-title"><?php echo $success; ?> <?php echo $prod_id; ?></h3>
