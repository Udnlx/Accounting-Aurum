...
if ($worker && $id_product_sell && $pay && $cash_card && $_SESSION['reload'] != 'on') {
    ...
    file_put_contents(__DIR__ . '/log_products_sell.txt', $log . PHP_EOL, FILE_APPEND);
                ...
                $log .= 'Операция проведена: ' . $worker . ', ID записи: ' . $cash_operation_id . ', Сумма: ' . $pay . ', Вид платежа: ' . $cash_card . ', Описание: Приход при продаже изделия по операции ID: ' . $product_page->id;
