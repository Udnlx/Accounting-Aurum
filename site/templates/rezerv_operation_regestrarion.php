...
$success = 'Регистрация продажи по резерву прошла успешно';
if ($worker && $proba && $weight && $price_gramm && $price && $pay && $cash_card && $_SESSION['reload'] != 'on') {
    ...
    $log = '';
    $log .= date("Y-m-d H:i") . ' Резерв - Закрытие - ' . $rezerv_page->proba . ' - ' . $rezerv_page->weight . 'г - ' . $rezerv_page->point . ' === ';
    $log .= 'Закрыл резерв: ' . $worker . ', ID резерва: ' . $rezerv_page->id . ', Комментарий резерва: ' . $rezerv_page->reserv_note; 
    file_put_contents(__DIR__ . '/log_reserv.txt', $log . PHP_EOL, FILE_APPEND);

    //Меняем таблицу резерва
    $point_actual_table = $pages->get('id_point=' . $rezerv_page->id_point . '_reserv');
    $edit_page = $point_actual_table->get('title=' . $rezerv_page->proba . '');
    // echo $edit_page . '<br>';
    // echo $edit_page->remain . '<br>';
    // echo $weight . '<br>';
    $result = $edit_page->remain - $rezerv_page->weight;
    // echo $result;
    $edit_page->of(false);
    ...
    'note' => 'Операция создана при закрытии резерва ID:' . $rezerv_page->id . ' - ' . $rezerv_page->title,
    ]);
    ...
    $log .= 'Операция создана при закрытии резерва ID:' . $rezerv_page->id . ' - ' . $rezerv_page->title;
...
?>
    <div id="content" style="max-width: 700px;">
        <h1 class="uk-heading-hero uk-text-center">Продажа лома по резерву - Регистрация продажи</h1>
...
<div id="content">
    <h1 class="uk-margin-remove uk-heading-hero uk-text-center">Продажа лома по резерву - Регистрация продажи</h1>
    <div>
